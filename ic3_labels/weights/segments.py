...
def generate_generator(dataset_number, n_files, outpath=None):
    if isinstance(dataset_number, Iterable) and isinstance(n_files, Iterable):
        if len(dataset_number) != len(
            ...
            print(
                "At least one of the present datasets of this type doesn't "
        ...
        if len(dataset_number) != len(n_files):
            raise ValueError(
                "Dataset_number and n_files have to be the same "
        ...
        isinstance(dataset_number, int) or isinstance(dataset_number, float)
    ) and (isinstance(n_files, int) or isinstance(n_files, float)):
        generator = from_simprod(dataset_number) * n_files
    else:
        raise ValueError(
            "Dataset_number and n_files either have to be both "
...
def calc_weights(frame, fluxes, flux_names, n_files, generator, key):
    ...
    if generator is not None:
        ...
        if frame.Has("CorsikaWeightMap"):
            ...
            if spectral_index == -1:
                energy_integral = np.log(cwm["EnergyPrimaryMax"]) - np.log(
                    cwm["EnergyPrimaryMin"]
                )
            else:
                energy_integral = (
        ...
        for flux, flux_name in zip(fluxes, flux_names):
            try:
                ...
                weight = (
                    flux_val * one_weight / (type_weight * n_events * n_files)
                )
                weight_dict[flux_name] = float(weight)
...
def calc_weights_muongun(tray, name, fluxes, flux_names, generator, key):
    ...
    for flux, flux_name in zip(fluxes, flux_names):
        ...
        tray.AddModule(
            update_weight_dict,
            "update_wd_{}".format(flux_name),
            frame_key=key,
            flux_name=flux_name,
        )
        tray.AddModule(
            "Delete", "delete_{}".format(flux_name), keys=[flux_name]
        )


@icetray.traysegment
def do_the_weighting(
    tray,
    name,
    fluxes,
    flux_names,
...
def WeightEvents(
    ...
    if dataset_type == "muongun":
        ...
        raise ValueError("Unknown dataset_type: {!r}".format(dataset_type))
    ...
    if perform_check:
        assert (
            n_files == dataset_n_files
        ), "N_files do not match: {!r} != {!r}".format(
            n_files, dataset_n_files
        )
        ...
        if (
            ...
            if self._last_n_events_per_run is not None:
                if meta["n_events_per_run"] != self._last_n_events_per_run:
                    raise ValueError(
                        "N events per Run changes: {} {}".format(
