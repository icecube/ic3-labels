...
def atmosphericFlux(
    neutrinoEnergy, neutrinoZenith, neutrinoType, atmFluxConv, atmFluxPrompt
):
    """
    ...
    badmask = neutrinoEnergy < 10.0
    if atmFluxConv is not None:
        conv = atmFluxConv.getFlux(
            p_types, neutrinoEnergy, np.cos(neutrinoZenith)
        )
        conv[badmask] = np.nan
        atmflux += conv
    ...
    if (isinstance(neutrinoZenith, float)) and (isinstance(neutrinoType, str)):
        for i in range(len(neutrinoEnergy)):
            if neutrinoEnergy[i] < 10.0:
                atmflux[i] = np.nan
                continue
...
class MESEWeights(icetray.I3ConditionalModule):
    ...
    def __init__(self, context):
        ...
        self.AddParameter(
            "DatasetType",
            "Type of dataset. Must be one of: " "'muongun', 'nugen', 'genie'",
        )
        self.AddParameter("DatasetNFiles", "Number of files")
        self.AddParameter(
            "DatasetNEventsPerRun", "Number of generated events per file"
        )
        self.AddParameter(
            "OutputKey", "Save weights to this frame key.", "MESE_weights"
        )
    ...
    def Configure(self):
        ...
        self.prompt_flux_multiplier = 0.2
    ...
    def Physics(self, frame):
        ...
        if self._dataset_type in ["nugen", "genie"]:
            ...
            mese_dict["weight_E269"] = (
                2.09e-18 * oneweight * (energy_true / 1e5) ** -2.69
            )
            mese_dict["weight_E250"] = (
                2.23e-18 * oneweight * (energy_true / 1e5) ** -2.5
            )

            # calculate atmospheric weights
            if is_tau:
                mese_dict["weight_conv"] = (
                    oneweight
                    * atmosphericFlux(
                    ...
                    * 2.0
                    * self.conv_flux_multiplier
                )

                mese_dict["weight_prompt"] = (
                    oneweight
                    * atmosphericFlux(
                        neutrinoEnergy=energy_true,
                        neutrinoZenith=zenith_true,
        ...
        try:
            ...
            # mask events
            track_mask = frame["is_cascade_reco"] | ~(
            ...
            # log_warn(e)
            print("Error in final track cut:", e)
            pass
        # -----------------------------------------------------
