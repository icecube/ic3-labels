...
class UnbrokenPowerLaw(object):
    ...
    def getFlux(self, ptype, energy, costheta):
        ...
        return (
            self.norm * np.power(energy / self.e_pivot, self.gamma) / n_types
        )


hese_flux = UnbrokenPowerLaw(-2.0, 0.95e-18)

# Global Fit Paper: https://arxiv.org/abs/1507.03991
# Divide the global_fit flux by 3 to obtain the flux per flavor
global_fit = UnbrokenPowerLaw(-2.5, 6.7e-18 / 3.0)

# Aachen Paper: https://arxiv.org/abs/1607.08006
aachen_flux = UnbrokenPowerLaw(-2.13, 0.9e-18)
...
# Most recent HESE-results: https://arxiv.org/pdf/1510.05223
hese4_fixed = UnbrokenPowerLaw(-2.0, 0.84e-18)
...
def get_fluxes_and_names(
    neutrinoflux_models="all", nnflux_models="all", measured_models="all"
):
