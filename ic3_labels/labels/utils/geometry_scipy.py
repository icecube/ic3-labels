...
def get_intersections(convex_hull, v_pos, v_dir, eps=1e-4):
    ...
    t_s = [
        ray_triangle_intersection(v_pos, v_dir, convex_hull.points[simp])
    ...
    if len(t_s) != 2:  # A line should have at max 2 intersection points
        ...
        t_s = np.hstack((t_s, t_s_back * (-1.0)))
...
def get_axial_cylinder_intersections(
    v_pos, v_dir, height=1000.0, radius=600.0, tol=1e-4
):
...
def point_is_inside(
    convex_hull, v_pos, default_v_dir=np.array([0.0, 0.0, 1.0]), eps=1e-4
):
...
def get_distance_to_edge(edge_point1, edge_point2, point):
    ...
    distance: float
    """
    closest_point = get_closest_point_on_edge(edge_point1, edge_point2, point)
...
def distance_to_axis_aligned_Volume(pos, points, z_min, z_max):
    ...
    edges = [
        (points[i], points[(i + 1) % (no_of_points)])
    ...
    for edge in edges:
        # x = (edge[0][0], edge[1][0])
        # y = (edge[0][1], edge[1][1])
        distance = get_distance_to_edge(edge[0], edge[1], [pos[0], pos[1], 0])
        t = get_edge_intersection(edge[0], edge[1], [pos[0], pos[1], 0])
        if not np.isnan(t):
            list_of_ts.append(t)
