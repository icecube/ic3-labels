...
def get_muon_bundle_information(frame, convex_hull, energy_threshold=20):
    ...
    energies_in_mctree = np.array([m.energy for m in muons])
    bundle_info['num_muons_in_mctree'] = len(muons)

    if len(muons) > 0:
        bundle_info['leading_energy_rel_mctree'] = np.max(
            energies_in_mctree) / np.sum(energies_in_mctree)
    else:
        bundle_info['leading_energy_rel_mctree'] = float('NaN')
