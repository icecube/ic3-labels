...
class EventGeneratorMultiCascadeLabels(MCLabelsBase):
    ...
    def __init__(self, context):
        ...
        self.AddParameter(
            "MCTreeName", "Name of the I3MCTree to use.", "I3MCTree"
        )
        self.AddParameter(
            "ExtendBoundary",
    ...
    def add_labels(self, frame):
        ...
        # that each cascade is injected as a daughter neutrino into
        ...
        neutrinos = [
            p for p in mc_tree.get_daughters(primary) if p.is_neutrino
        ]
        for i, neutrino in enumerate(neutrinos):
            ...
            labels[cascade_name + "energy_fraction_hadron"] = (
                e_hadron / e_total
            )
