...
class EventGeneratorMuonTrackLabels(MCLabelsBase):
    ...
    def add_labels(self, frame):
        ...
        if len(update_distances) > 0:
            ...
            rel_distances = update_distances[1:] - (
                track_anchor.pos - muon.pos).magnitude
        ...
        for i, cascade in enumerate(cascades):
            ...
            # calculate distance to highes charge cascade
        ...
        labels['track_distance_start'] = (
            (track_start.pos - muon.pos).magnitude -
            (track_anchor.pos - muon.pos).magnitude
        )
        labels['track_distance_end'] = (
            (track_end.pos - muon.pos).magnitude -
            (track_anchor.pos - muon.pos).magnitude
        )
